...
class Menu:
    def __init__(self, screen_width: int = 800,
        ...
        self.__start_rect = self.__start.get_rect()
        self.__start_rect.x = 250
        self.__start_rect.y = 170    
        ...
        self.__exit_button = self.__exit_text.get_rect()
        self.__exit_button.x = 100
        self.__exit_button.y = 370
        
        #максимальное колво очков
        self.__font_comicsans_ms = pygame.font.SysFont("comicsansms", 20)
        with open("data/max_score_tc.txt", "r") as file:
            self.__mscore_txt = str(file.read().strip())
       
        self.__max_score_text = self.__font_comicsans_ms.render(f"{self.__mscore_txt}", True, (255,255,255))
       
        self.__max_score_rect = self.__max_score_text.get_rect()
        self.__max_score_rect.x = 40
        self.__max_score_rect.y = 40
